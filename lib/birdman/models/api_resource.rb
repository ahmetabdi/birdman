...
private
  ...
  def self.build_single_resource(resource_from_response, id = nil)
    resource = self.new
    resource.public_send("id=", id) if id
  ...
  def self.build_single_resource_with_id(resource_from_response, id)
    resource = self.new
    self::RESOURCE_ATTRIBUTES.each do |resource_attr|
      resource.public_send("#{resource_attr}=", resource_from_response.public_send(resource_attr)) if resource_from_response.respond_to?(resource_attr)
    end
